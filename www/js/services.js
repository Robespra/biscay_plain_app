...
function Item(entry, pub_name, feed_link) {
  ...
  if (entry) {
	  ...
	  this.pub_author = entry.author.name;
	  this.pub_date = new Date(entry.date);
	  this.item_link = entry.link;
...
services.factory('items', ['$http', 'store', 'filterFilter', function($http, store, filter) {
	var items = {
	  ...
	  getItemsFromServer: function() {
	    ...
	    var feedURL = 'http://www.poiesis-app.com/wp-json/posts?filter[post_status]=publish&filter[posts_per_page]=200&filter[orderby]=date&filter[order]=desc';
	    // var feedURL = 'http://localhost:8080/biscay_plain_app/www/contenidojson.js';
	    var successCallback = function(data, status, headers, config) {
	      ...
	      data.forEach(function(entry, index, array) {
	      	...
	      	var item = new Item(entry, 'Poeisis', 'http://www.poiesis-app.com');
		...
		var errorCallback = function(data, status, headers, config) {
			console.log ("Something wrong with calling JSON");
		}
